...
### `r paste0("Affected AUS States at ", aus_recent_sum$Date)` 

```{r affect_state}
affect_state <- NROW(state_recent)
valueBox(prettyNum(affect_state, big.mark = ","), icon = "ion-android-globe", color = '#835bd4')
```
...
anim_save("image/animate_aus_increment.gif", animation = animate(animate_aus_increment, nframes = 180, fps = 10, end_pause = 50))
```
...
```{r table_state}
state_recent %>%
  select(Country, State, Date, Confirmed, Recovered, Deaths) %>%
  arrange(desc(Confirmed)) %>%
  datatable(
    rownames = FALSE,
    fillContainer = TRUE,
...
anim_save("image/animate_confirmed.gif", animation = animate(animate_confirmed, nframes = 150, fps = 10, end_pause = 50))
```
...
anim_save("image/animate_recovered.gif", animation = animate(animate_recovered, nframes = 150, fps = 10, end_pause = 50))
```
...
anim_save("image/animate_deaths.gif", animation = animate(animate_deaths, nframes = 150, fps = 10, end_pause = 50))
```
